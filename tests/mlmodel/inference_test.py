...
class TestModuleConstants(unittest.TestCase):
    def test_the_light_curve_length_matches_the_padding_target(self):
        ...
        self.assertEqual(inference.num_points, dataprocessing.num_points)
...
@needs_torch
class TestCastAsBilbyResult(CastResultMixin, unittest.TestCase):
    def test_a_bilby_result_is_returned(self):
        self.assertIsInstance(self.cast(), bilby.result.Result)
    ...
    def test_the_three_ejecta_parameters_become_the_posterior_columns(self):
        ...
        self.assertEqual(set(self.cast().posterior.columns), set(PARAMETERS))

    def test_one_posterior_row_is_produced_per_sample(self):
        self.assertEqual(len(self.cast().posterior), 3)
    ...
    def test_an_extra_leading_axis_is_flattened_away(self):
        ...
        self.assertEqual(len(result.posterior), 5)


@needs_torch
class TestCastAsBilbyResultWithoutTruth(CastResultMixin, unittest.TestCase):
    """A run on real data has no injected values to compare against."""

    def test_no_injection_parameters_are_recorded(self):
        self.assertIsNone(self.cast(truth=None).injection_parameters)

    def test_the_posterior_is_still_built(self):
        self.assertEqual(len(self.cast(truth=None).posterior), 3)


@needs_torch
class TestCastAsBilbyResultWithTruth(CastResultMixin, unittest.TestCase):
    ...
    def test_the_injected_values_are_recorded(self):
        ...
        self.assertIsNotNone(result.injection_parameters)
        self.assertEqual(set(result.injection_parameters), set(PARAMETERS))

    def test_each_injected_value_maps_to_its_own_parameter(self):
        injected = self.cast(truth=self.truth).injection_parameters
        self.assertAlmostEqual(injected["log10_mej"], -1.2, places=5)
        self.assertAlmostEqual(injected["log10_vej"], -0.55, places=5)
        self.assertAlmostEqual(injected["log10_Xlan"], -2.2, places=5)
    ...
    def test_a_tensor_truth_is_recognised_as_given(self):
        ...
        self.assertIsNotNone(self.cast(truth=self.truth).injection_parameters)
