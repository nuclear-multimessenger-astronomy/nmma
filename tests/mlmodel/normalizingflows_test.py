...
needs_torch = pytest.mark.skipif(
    not TORCH_AVAILABLE,
    reason="torch or nflows is not installed; install the neuralnet extra",
)
...
class TestModuleConstants:
    ...
    def test_the_context_width_matches_the_representation_width(self):
        assert normalizingflows.num_dim == 7
        assert normalizingflows.context_features == normalizingflows.num_dim

    def test_the_light_curve_length_matches_the_padding_target(self):
        from nmma.mlmodel import dataprocessing

        assert normalizingflows.num_points == dataprocessing.num_points

    def test_a_module_level_embedding_is_built_at_import(self):
        assert isinstance(normalizingflows.similarity_embedding, SimilarityEmbedding)

    def test_the_device_is_resolved_at_import(self):
        assert isinstance(normalizingflows.device, torch.device)
...
class TestEmbeddingNet:
    ...
    def test_the_light_curve_is_reduced_to_the_context_vector(self):
        ...
        assert tuple(network(torch.randn(4, 3, 121)).shape) == (4, 7)

    def test_the_batch_size_is_taken_from_the_input(self):
        network = self.build().eval()
        assert network(torch.randn(9, 3, 121)).shape[0] == 9

    def test_the_wrapped_embedding_is_kept_as_a_submodule(self):
        network = self.build()
        assert isinstance(network.representation_net, SimilarityEmbedding)
    ...
    def test_a_context_layer_is_added_on_top(self):
        network = self.build()
        assert isinstance(network.context_layer, nn.Sequential)
        assert network.context_layer[0].in_features == 7
        assert network.context_layer[-1].out_features == 7

    def test_the_output_is_differentiable_through_the_context_layer(self):
        network = self.build()
        network(torch.randn(2, 3, 121)).sum().backward()
        assert network.context_layer[0].weight.grad is not None
...
class TestNormflowParams:
    ...
    def test_three_pieces_are_returned(self):
        ...
        assert isinstance(base_dist, StandardNormal)
        assert isinstance(embedding_net, normalizingflows.EmbeddingNet)

    def test_the_base_distribution_is_over_the_three_ejecta_parameters(self):
        _, base_dist, _ = self.build()
        assert tuple(base_dist._shape) == (3,)
    ...
    def test_each_transform_is_paired_with_a_permutation(self):
        ...
        assert len(transform._transforms) == 6
    ...
    def test_no_transforms_gives_an_empty_stack(self):
        ...
        assert len(transform._transforms) == 0
    ...
    def test_the_pieces_assemble_into_a_working_flow(self):
        ...
        assert tuple(samples.shape) == (2, 5, 3)
...
@needs_torch
class TestTrainOneEpoch(FlowLoopMixin):
    ...
    def test_a_loss_is_reported_once_ten_batches_have_been_seen(self):
        assert self.run_epoch(10) == pytest.approx(2.0, abs=1.5e-3)
    ...
    def test_the_weights_are_updated(self):
        ...
        self.run_epoch(10)
        assert before != float(self.flow.scale)
    ...
    def test_the_running_loss_is_reported_per_batch_not_accumulated(self):
        ...
        self.optimizer = torch.optim.SGD(self.flow.parameters(), lr=0.0)
        assert self.run_epoch(10) == pytest.approx(5.0, abs=1.5e-3)
...
@needs_torch
class TestValidateOneEpoch(FlowLoopMixin):
    def run_epoch(self, count, flatten_dim=1):
    ...
    def test_a_loss_is_reported_after_every_batch(self):
        ...
        assert self.run_epoch(1) == pytest.approx(2.0, abs=1.5e-3)

    def test_every_batch_is_written_to_tensorboard(self):
        self.run_epoch(3)
        assert self.writer.add_scalar.call_count == 3
        assert self.writer.add_scalar.call_args.args[0] == "Flow Loss/val"
