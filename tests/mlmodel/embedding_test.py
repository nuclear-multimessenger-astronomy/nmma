...
class TestModuleConstants:
    ...
    def test_the_three_ztf_bands_are_the_channels(self):
        assert embedding.bands == ["ztfg", "ztfr", "ztfi"]
        assert embedding.num_channels == len(embedding.bands)

    def test_the_light_curve_length_matches_the_padding_target(self):
        from nmma.mlmodel import dataprocessing

        assert embedding.num_points == dataprocessing.num_points

    def test_the_detection_limit_is_the_padding_value(self):
        from nmma.mlmodel import dataprocessing

        assert embedding.detection_limit == dataprocessing.detection_limit


@needs_torch
class TestVICRegLossOffDiagonal:
    ...
    def test_the_diagonal_is_dropped_from_each_matrix(self):
        ...
        assert self.loss.off_diagonal(covariance).tolist() == [
            1.0,
            2.0,
            3.0,
            5.0,
    ...
    def test_a_diagonal_matrix_has_no_off_diagonal_content(self):
        ...
        assert float(self.loss.off_diagonal(covariance).abs().sum()) == 0.0
...
class TestVICRegLoss:
    ...
    def test_the_covariance_term_is_never_negative(self):
        ...
        assert float(cov_loss) >= 0.0


@needs_torch
class TestConvResidualBlock:
    def test_the_shape_is_preserved(self):
        block = embedding.ConvResidualBlock(channels=8, kernel_size=5).eval()
        assert tuple(block(torch.randn(2, 8, 32)).shape) == (2, 8, 32)
    ...
    def test_batch_normalisation_is_on_by_default(self):
        block = embedding.ConvResidualBlock(channels=8, kernel_size=5)
        assert block.use_batch_norm
        assert len(block.batch_norm_layers) == 2
    ...
    def test_batch_normalisation_can_be_switched_off(self):
        ...
        assert not hasattr(block, "batch_norm_layers")
    ...
    def test_the_last_convolution_starts_near_zero_so_the_block_is_an_identity(self):
        ...
        assert float(block.conv_layers[-1].weight.abs().max()) < 1e-3
    ...
    def test_dropout_is_applied_during_training_only(self):
        ...
        assert block.dropout.p == pytest.approx(0.9)


@needs_torch
class TestConvResidualNet:
...
class TestSimilarityEmbedding:
    ...
    def test_both_the_expanded_output_and_the_representation_are_returned(self):
        ...
        assert tuple(expanded.shape) == (4, 5)
        assert tuple(representation.shape) == (4, 7)
    ...
    def test_the_backbone_is_the_one_dimensional_resnet(self):
        assert isinstance(self.build().layers_f, ResNet)
...
@needs_torch
class TestTrainOneEpoch(TrainingLoopMixin):
    ...
    def test_the_loss_weights_are_forwarded(self):
        ...
        assert loss.call_args.kwargs == {"wt_cov": 2.0}
    ...
    def test_the_tensorboard_step_counts_across_epochs(self):
        ...
        assert self.writer.add_scalar.call_args.args[2] == 3 * 10 + 10
    ...
    def test_verbose_mode_reports_the_three_loss_terms(self):
        ...
        assert "train loss/batch" in stream.getvalue()


@needs_torch
class TestValidateOneEpoch(TrainingLoopMixin):
    ...
    def test_a_loss_is_reported_after_every_batch(self):
        ...
        assert self.run_epoch(1) > 0.0

    def test_every_batch_is_written_to_tensorboard(self):
        self.run_epoch(3)
        assert self.writer.add_scalar.call_count == 3
        assert self.writer.add_scalar.call_args.args[0] == "SimLoss/val"
    ...
    def test_the_writer_is_flushed_so_the_epoch_is_visible(self):
        self.run_epoch(1)
        assert self.writer.flush.called

    def test_an_empty_loader_reports_no_loss(self):
        assert self.run_epoch(0) == 0.0
