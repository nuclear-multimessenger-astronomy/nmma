...
class TestH0Resampling(unittest.TestCase):
    ...
    def test_the_requested_number_of_samples_is_returned(self):
        ...
        self.assertEqual(samples.shape, (321,))
    ...
    def test_the_draws_follow_the_weighting(self):
        ...
        self.assertLess(np.median(low), np.median(flat))

    def test_the_draws_stay_in_a_plausible_range(self):
        samples = hubble_estimates.H0_resampling(self.prior, self.weights, 2000)
        self.assertGreater(samples.min(), -50)
        self.assertLess(samples.max(), 250)


class TestGenerateLogprob(unittest.TestCase):
    ...
    def test_one_row_is_produced_per_event(self):
        ...
        self.assertEqual(rows.shape, (3, len(self.samples)))
    ...
    def test_each_row_is_normalised_to_sum_to_one(self):
        ...
        for row in rows:
            self.assertAlmostEqual(np.exp(row).sum(), 1.0, places=6)
    ...
    def test_a_volume_weighting_is_applied_from_the_second_event_onwards(self):
        ...
        self.assertFalse(np.allclose(single[0], pair[-1]))
...
class TestGetCumprodRowwise(unittest.TestCase):
    ...
    def test_the_shape_is_preserved(self):
        ...
        self.assertEqual(hubble_estimates.get_cumprod_rowwise(logprob).shape, (3, 4))
...
class TestGenerateCumprods(unittest.TestCase):
    ...
    def test_each_set_has_one_weighting_per_event(self):
        ...
        for weighting in [gw, em, total]:
            self.assertEqual(weighting.shape, (2, len(self.samples)))
    ...
    def test_the_combined_weighting_differs_from_either_messenger(self):
        ...
        self.assertFalse(np.allclose(total[-1], gw[-1]))
        self.assertFalse(np.allclose(total[-1], em[-1]))

    def test_every_weighting_is_non_negative(self):
        for weighting in hubble_estimates.generate_cumprods(
            self.gw, self.em, self.samples, [0, 1]
        ):
            self.assertTrue(np.all(weighting >= 0.0))


class TestH0MeansFromProbs(unittest.TestCase):
    ...
    def test_one_spread_is_computed_per_messenger_per_ordering(self):
        ...
        self.assertEqual(spread.call_count, 3 * 3)
    ...
    def test_the_event_order_is_reshuffled_for_every_realisation(self):
        ...
        self.assertEqual(shuffler.shuffle.call_count, 3)
    ...
    def test_the_credible_interval_is_passed_through(self):
        ...
        self.assertAlmostEqual(spread.call_args.args[4], 0.68)
...
class TestLoadInPosteriors(LoadPosteriorsMixin, unittest.TestCase):
    def test_one_density_per_event_is_built_for_each_messenger(self):
        ...
        self.assertEqual(sorted(em), [0, 1, 2])
        self.assertEqual(sorted(gw), [0, 1, 2])

    def test_the_densities_are_over_the_hubble_constant(self):
        em, _ = hubble_estimates.load_in_posteriors(
            self.injection, range(self.n_events), self.args()
        )
        self.assertIsInstance(em[0], scipy.stats.gaussian_kde)
        self.assertGreater(em[0].dataset.mean(), 10.0)
        self.assertLess(em[0].dataset.mean(), 200.0)
    ...
    def test_a_missing_event_file_is_skipped_rather_than_fatal(self):
        ...
        self.assertEqual(sorted(gw), [0, 2])
        self.assertEqual(sorted(em), [0, 2])
    ...
    def test_the_electromagnetic_density_is_reweighted_by_distance_squared(self):
        ...
        self.assertIsNotNone(em[0].weights)
        self.assertFalse(np.allclose(em[0].weights, em[0].weights[0]))
...
class TestMain(LoadPosteriorsMixin, unittest.TestCase):
    ...
    def test_the_trend_file_is_written(self):
        self.run_main()
        self.assertTrue(Path("GW_EM_H0_trend_run.dat").is_file())
    ...
    def test_the_trend_file_lands_in_the_working_directory_not_the_outdir(self):
        ...
        self.run_main()
        self.assertFalse((self.tmp_dir / "GW_EM_H0_trend_run.dat").is_file())
        self.assertTrue(Path("GW_EM_H0_trend_run.dat").is_file())
        Path("GW_EM_H0_trend_run.dat").unlink()
    ...
    def test_the_trend_file_holds_a_column_per_messenger_and_bound(self):
        ...
        for column in [
            ...
            self.assertIn(column, written.columns, msg=column)
        Path("GW_EM_H0_trend_run.dat").unlink()
...
class TestScipySpecialImport(unittest.TestCase):
    def test_the_special_submodule_is_reachable_without_being_imported(self):
        ...
        self.assertIn("scipy.special.logsumexp", source)
        self.assertNotIn("import scipy.special", source)
        self.assertTrue(hasattr(scipy, "special"))
