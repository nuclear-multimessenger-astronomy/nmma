...
class TestSetupEoSGenerator:
    ...
    def test_dispatches_each_known_model_name(self):
        ...
        for model_name, class_name in cases.items():
            ...
            assert returned is generator_class.return_value, model_name
    ...
    def test_the_model_name_is_case_insensitive(self):
        ...
        assert returned is generator_class.return_value
    ...
    def test_an_unknown_model_name_is_reported(self):
        ...
        assert "unsupported" in str(context.value)
...
class TestEoSGeneratorKerasBackend(KerasEmulatorMixin):
    ...
    def test_loading_selects_a_backend_specific_predict(self):
        ...
        if expected is not None:
            assert generator.predict == expected

    def test_explicit_eos_parameters_override_the_class_attribute(self):
        generator = eos_processing.EoSGenerator(
            str(self.emulator_path), eos_parameters=["a", "b"]
        )
        assert generator.eos_parameters == ["a", "b"]
        assert eos_processing.EoSGenerator.eos_parameters is None

    def test_default_mass_construction_is_equally_spaced(self):
        generator = eos_processing.EoSGenerator(str(self.emulator_path))
        assert generator.n_mass_samples == 30
        assert generator.decompose_mass_data == generator.equal_distance_masses
...
class TestEoSGeneratorPickleFallback:
    ...
    def test_a_pickled_emulator_is_loaded_and_uses_pickle_predict(self):
        ...
        assert generator.predict == generator.pickle_predict
        assert isinstance(generator.emulator, ConstantEmulator)
...
class TestNEPEoSGenerator(KerasEmulatorMixin):
    ...
    def test_metadata_drives_the_emulator_path_and_mass_sampling(self):
        ...
        assert generator.n_mass_samples == self.n_mass_samples
        assert generator.decompose_mass_data == generator.equal_distance_masses
    ...
    def test_the_default_number_of_mass_samples_is_forty(self):
        ...
        assert generator.n_mass_samples == 40
    ...
    def test_nep5_declares_the_five_nuclear_empirical_parameters(self):
        assert eos_processing.NEP5EoSGenerator.eos_parameters == [
            "K_sat",
            "L_sym",
            "K_sym",
            "3n_sat",
    ...
    def test_a_split_mass_grid_is_configured_from_a_three_element_list(self):
        ...
        assert generator.mass_samples_low == 3
        assert generator.mass_samples_high == 2
        assert generator.split_value == 2.0
        assert generator.n_mass_samples == 5
        assert generator.decompose_mass_data == generator.disjoint_masses
...
class TestLECEoSGenerator:
    ...
    def test_lec7_declares_the_seven_low_energy_couplings(self):
        assert eos_processing.LEC7EoSGenerator.eos_parameters == [
            "d11",
            "d22",
            "d3",
            "d4",
    ...
    def test_lec13_extends_the_couplings_with_saturation_parameters(self):
        ...
        assert parameters[:6] == eos_processing.LEC7EoSGenerator.eos_parameters
        assert parameters[6:] == [
            "ksat",
            "qsat",
            "zsat",
    ...
    def test_all_six_joblib_artefacts_are_loaded(self):
        ...
        for attribute in [
            ...
            assert hasattr(generator, attribute), attribute
...
class TestEoSConverterMethodSelection:
    ...
    def test_an_eos_file_selects_the_single_tabulated_eos_path(self):
        ...
        assert converter.macro_conversion == converter.single_eos_from_ram
    ...
    def test_an_eos_directory_selects_the_tabulated_set_path(self):
        ...
        converter = eos_processing.EoSConverter(args)
        assert converter.macro_conversion == converter.eos_from_ram
    ...
    def test_emulator_metadata_selects_the_emulated_path(self):
        ...
        assert converter.tov_emulator is setup.return_value
        assert converter.macro_conversion == setup.return_value.generate_macro_eos
    ...
    def test_the_quasi_universal_relation_path_replaces_the_conversion(self):
        ...
        assert converter.parameter_conversion is eos_processing.radii_from_qur
    ...
    def test_the_quasi_universal_relation_path_derives_radii_from_lambdas(self):
        ...
        for key in ["radius_1", "radius_2", "R_16"]:
            ...
            assert 8.0 < float(converted[key]) < 16.0, key
    ...
    def test_the_default_parameter_conversion_is_the_full_chain(self):
        args = Namespace(eos_file=str(self.tmp_dir / "1.dat"), emulator_metadata=None)
        converter = eos_processing.EoSConverter(args)
        assert converter.parameter_conversion == converter.full_eos_conversion


class TestEoSConverterTabulatedSet:
    ...
    def test_all_requested_files_are_loaded_into_memory(self):
        ...
        assert converter.Neos == self.n_eos
        assert len(converter.eos_data) == self.n_eos
    ...
    def test_files_are_read_from_disk_when_they_are_not_kept_in_ram(self):
        ...
        converter = eos_processing.EoSConverter(args)
        assert converter.macro_conversion == converter.eos_direct_load
        assert Path(converter.eos_data) == self.tmp_dir
...
class TestEoSConverterParameterConversion:
    ...
    def test_a_single_eos_yields_scalar_neutron_star_parameters(self):
        ...
        assert parameters["TOV_mass"] == pytest.approx(masses[-1])
        assert parameters["TOV_radius"] == pytest.approx(radii[-1])
        assert parameters["R_14"] == pytest.approx(np.interp(1.4, masses, radii))
        assert parameters["R_16"] == pytest.approx(np.interp(1.6, masses, radii))
    ...
    def test_the_selected_macro_eos_is_cached_for_later_use(self):
        ...
        assert sorted(cached) == ["lambdas", "masses", "radii"]
    ...
    def test_several_eos_indices_yield_arrays_of_neutron_star_parameters(self):
        ...
        assert parameters["R_14"].shape == (2,)
    ...
    def test_system_parameters_are_interpolated_at_the_component_masses(self):
        ...
        assert converted["radius_1"] == pytest.approx(np.interp(1.6, masses, radii))
        assert converted["radius_2"] == pytest.approx(np.interp(1.2, masses, radii))
        assert converted["lambda_1"] == pytest.approx(
            np.exp(np.interp(1.6, masses, np.log(lambdas)))
        )
        assert converted["lambda_2"] > converted["lambda_1"]
    ...
    def test_a_component_mass_above_the_tov_mass_gives_a_vanishing_lambda(self):
        ...
        converted = self.converter.system_props_from_eos(parameters)
        assert converted["lambda_1"] == 0.0
        assert converted["radius_1"] == 0.0
    ...
    def test_calling_the_converter_runs_the_full_chain(self):
        ...
        for key in ["TOV_mass", "R_14", "lambda_1", "lambda_2", "radius_1", "radius_2"]:
            assert np.shape(converted[key]) == (2,), key
    ...
    def test_the_single_eos_path_ignores_the_eos_index(self):
        ...
        masses, radii, _ = self.tables[0]
        assert converted["TOV_mass"] == pytest.approx(masses[-1])
        assert converted["radius_1"] == pytest.approx(np.interp(1.4, masses, radii))


class TestTabulatedEoSLoaders:
    ...
    def test_load_eos_files_sorts_the_directory_and_counts_the_files(self):
        ...
        assert [path.name for path in files] == ["1.dat", "2.dat", "3.dat"]
        assert n_eos == 3
    ...
    def test_load_weights_reads_a_file_and_passes_arrays_through(self):
        ...
        assert eos_processing.load_weights(weights) is weights
        assert eos_processing.load_weights(None) is None
    ...
    def test_load_to_dict_keys_the_set_by_a_one_based_index(self):
        ...
        assert sorted(eos_data) == [1, 2, 3]
        assert sorted(eos_data[1]) == ["Lambda", "M", "R"]
    ...
    def test_load_to_dict_attaches_weights_when_they_are_given(self):
        ...
        assert eos_data[3]["weight"] == pytest.approx(0.5)
    ...
    def test_load_to_list_returns_one_array_per_eos(self):
        ...
        assert len(eos_data) == 3
        assert eos_data[0].shape[0] == 3
