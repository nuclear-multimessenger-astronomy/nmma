...
class TestGetModelsHome(ModelsHomeMixin, unittest.TestCase):
    ...
    def test_an_explicit_path_beats_the_environment_variable(self):
        ...
        self.assertEqual(gitlab.get_models_home(other), other)
    ...
    def test_a_string_path_is_accepted(self):
        self.assertEqual(gitlab.get_models_home(str(self.models_home)), self.models_home)

    def test_the_default_lives_next_to_the_package(self):
        self.assertEqual(gitlab.DEFAULT_MODELS_HOME.name, "nmma_models")


class TestClearDataHome(ModelsHomeMixin, unittest.TestCase):
    def test_the_directory_is_removed(self):
        self.write_models_yaml()
        gitlab.clear_data_home(self.models_home)
        self.assertFalse(self.models_home.exists())


class TestDownload(ModelsHomeMixin, unittest.TestCase):
    ...
    def test_chunked_content_is_reassembled(self):
        ...
        self.assertEqual(target.read_bytes(), b"aabbcc")
...
class TestDecompress(ModelsHomeMixin, unittest.TestCase):
    ...
    def test_the_external_decompressor_is_invoked(self):
        ...
        self.assertEqual(mock_popen.call_args[0][0], ["lzma", "-d", path])
    ...
    def test_an_already_decompressed_file_is_tolerated(self):
        ...
        with patch.object(gitlab.subprocess, "Popen", return_value=process):
            self.assertEqual(gitlab.decompress(path), path)
...
class TestModelsList(ModelsHomeMixin, unittest.TestCase):
    def test_the_models_list_is_fetched_into_the_models_home(self):
        ...
        self.assertEqual(
            (self.models_home / "models.yaml").read_bytes(), b"Ka2017:\n"
        )
        self.assertIn("models.yaml", mock_get.call_args[0][0])
    ...
    def test_a_failed_download_falls_back_to_the_local_files(self):
        ...
        self.assertIn("Bu2019lm", models)
        self.assertEqual(models["Bu2019lm"]["filters"], ["sdss_g"])
    ...
    def test_locally_present_filters_are_added_to_the_downloaded_list(self):
        ...
        self.assertEqual(set(models["Ka2017"]["filters"]), {"sdss_g", "sdss_r"})
    ...
    def test_the_model_name_prefix_is_stripped_from_local_filter_files(self):
        ...
        models, _ = gitlab.load_models_list(self.models_home)
        self.assertEqual(models["Ka2017"]["filters"], ["sdss_g"])
    ...
    def test_refresh_drops_the_cached_list_and_reloads_it(self):
        ...
        self.assertIn("Bu2019lm", models)
        self.assertNotIn("Ka2017", models)
    ...
    def test_refresh_updates_the_module_level_cache(self):
        ...
        self.assertIn("Bu2019lm", gitlab.MODELS)
...
class TestGetModel(ModelsHomeMixin, unittest.TestCase):
    def setUp(self):
        super().setUp()
        self.write_models_yaml(
            "Ka2017:\n  filters:\n    - sdss_g\n    - sdss_r\n"
    ...
    def test_the_core_model_and_every_filter_are_returned(self):
        ...
        self.assertEqual(filters, ["sdss_g", "sdss_r"])
        self.assertEqual(len(filepaths), 3)
        self.assertTrue(filepaths[0].endswith("Ka2017.joblib"))
    ...
    def test_colon_separated_filter_synonyms_are_accepted(self):
        ...
        self.assertEqual(filters, ["sdss:g"])
    ...
    def test_x_ray_and_radio_filters_are_not_downloaded_but_still_reported(self):
        ...
        self.assertEqual(filters, ["sdss_g", "X-ray-1keV"])
        self.assertFalse(any("X-ray" in p for p in filepaths))

    def test_a_tensorflow_model_uses_h5_filter_files(self):
        filepaths, _, _, _ = self.call_get_model(model_name="Ka2017_tf")
        self.assertTrue(any(p.endswith("sdss_g.h5") for p in filepaths))
    ...
    def test_a_filter_missing_from_the_local_list_is_reported_as_such(self):
        ...
        self.assertIn("local models list", str(raised.exception))


class TestCommandLineInterface(ModelsHomeMixin, unittest.TestCase):
    def test_the_parser_exposes_the_documented_flags(self):
        args = gitlab.get_parser().parse_args([])
        self.assertIsNone(args.model)
        self.assertIsNone(args.svd_path)
        self.assertIsNone(args.filters)
        self.assertFalse(args.refresh_models_list)

    def test_arguments_are_parsed(self):
        args = gitlab.get_parser().parse_args(
            ["--model", "Ka2017", "--filters", "sdss_g,sdss_r", "--refresh-models-list"]
        )
        self.assertEqual(args.model, "Ka2017")
        self.assertEqual(args.filters, "sdss_g,sdss_r")
        self.assertTrue(args.refresh_models_list)
    ...
    def test_main_splits_the_filter_list_and_forwards_the_request(self):
        ...
        self.assertEqual(kwargs["filters"], ["sdss_g", "sdss_r"])
        self.assertEqual(kwargs["model_name"], "Ka2017")
        self.assertEqual(kwargs["models_home"], str(self.models_home))
    ...
    def test_an_empty_svd_path_falls_back_to_the_default(self):
        ...
        with patch.object(gitlab, "get_model", return_value=([], [])) as mock_get_model:
            gitlab.main(args)
        self.assertIsNone(mock_get_model.call_args.kwargs["models_home"])
