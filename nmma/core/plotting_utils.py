...
def plotting_parameters_from_priors(priors, keys=None):
    ...
    return {
        ...
        if k in keys and not isinstance(v, DeltaFunction)
    }


def setup_multi_axes(
    num_axes, sharex=False, sharey=False, ncols=None, dpi=250, **fig_kwargs
):
...
def arange_titles(ax, move=False):
    ...
    if len(ax.texts) == 2:
        ax.texts[0].set_position((0.0, lower_y))
        ax.texts[0].set_ha("left")
        ax.texts[1].set_position((1.0, lower_y))
        ...
        move_up(ax.texts[1])
        ax.texts[2].set_position((1.0, lower_y))
        ax.texts[2].set_ha("right")
        if move:
            move_up(ax.texts[0], amount=0.44)
    elif len(ax.texts) == 4:
        move_up(ax.texts[0])
        ax.texts[1].set_position((0.0, lower_y))
        ax.texts[1].set_ha("left")
        ax.texts[2].set_position((1.0, lower_y))
        ax.texts[2].set_ha("right")
        move_up(ax.texts[2])
        ax.texts[3].set_position((1.0, lower_y))
...
def texts_overlap(left_text, right_text):
    ...
    return (
        True
        if (left_bounds[1, 0] > right_bounds[0, 0])
        and (left_bounds[1, 1] > right_bounds[0, 1])
        and (left_bounds[1, 1] <= right_bounds[1, 1])
        else False
    )
