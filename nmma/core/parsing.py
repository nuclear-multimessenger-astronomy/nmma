...
def check_for_config(cli_args, parents=[], drop_config=True):
    ...
    if cli_args:
        ...
        if (
            first_arg.startswith("-c")
            or first_arg.startswith("--config")
            or first_arg.startswith("--ini")
        ):
            cli_args.pop(0)
            first_arg = cli_args[0]
            config_given = True
...
def base_analysis_parsing(parser):
    ...
    parser.add_argument(
        "--Hubble-weight", help="Path to the precalculated Hubble weighting"
    )
...
def single_messenger_analysis_parsing(parser):
    ...
    parser.add_argument(
        ...
        type=yaml_parse,
        default={},
        help="Additional keyword arguments to pass to the plotting routine as a dictionary",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--result-format", default="json", help="Format to save the result"
    )
...
def base_injection_parsing(parser):
    ...
    parser.add_argument(
        ...
        "--outfile-type",
        default="json",
        choices=["json", "dat", "csv"],
        help="Injection file format",
    )
    parser.add_argument(
        "--generation-seed",
        type=int,
    ...
    parser.add_argument(
        "--injection",
        action="store_true",
        help="Whether to generate injections from prior (default: False)",
    )
    parser.add_argument(
        "--injection-num",
        type=int,
...
def pipe_inj_parsing(parser):
    ...
    parser.add_argument(
        "--prior-dict",
        help=(
            "A prior dict to use for generating injections. If not given, the prior file is used. "
        ),
    )
    parser.add_argument(
        "-n",
        ...
        default=20,
        help=(
            "The number of injections to generate: not required"
    ...
    parser.add_argument(
        ...
        "--gps-file",
        help=(
            "A list of gps start times to use for setting a geocent_time"
...
def slurm_analysis_parser(parser):
    ...
    slurm_args.add_argument(
        "--Ncore", default=8, type=int, help="number of cores for mpiexec"
    )
    slurm_args.add_argument(
        "--base-dir", default=Path.cwd(), type=Path, help="base directory for the job"
    )
    slurm_args.add_argument("--job-name")
    slurm_args.add_argument(
        "--logs-dir-name", default="slurm_logs", help="directory name for slurm logs"
    )
    slurm_args.add_argument(
        "--cluster-name", default="Expanse", help="Name of HPC cluster"
    )
    slurm_args.add_argument(
        "--partition-type",
        default="shared",
    ...
    slurm_args.add_argument(
        "--nodes", type=int, default=1, help="Number of nodes to request for computing"
    )
    slurm_args.add_argument(
    ...
    slurm_args.add_argument("--time", default="24:00:00", help="Walltime for instance")
    slurm_args.add_argument(
        "--mail-type", default="NONE", help="slurm mail type (e.g. NONE, FAIL, ALL)"
    )
...
def process_sampler_kwargs(args):
    ...
    run_sampler_kwargs = {
        key: getattr(args, key, default_kwargs[key]) for key in ["dlogz", "save_bounds"]
    }

    def_init_kwargs = {
        key: getattr(args, key, default_kwargs[key])
        for key in ["min_eff", "sample", "nlive", "bound", "walks", "facc", "enlarge"]
    }

    sampler_init_kwargs = def_init_kwargs | args.sampler_kwargs
    sampler_init_kwargs["first_update"] = dict(
        min_eff=sampler_init_kwargs.pop("min_eff"), min_ncall=2 * args.nlive
    )
