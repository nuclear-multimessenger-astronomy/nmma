    ...
    **kwargs,
    ...
    if not shared_data or not fig:
        ...
        add_xlim = (
            check_limit(xlim)
            if xlim
            else get_time_limits_from_obs_data(transient, filter_names)
        )
        ...
        for cnt, filt in enumerate(filter_names):
            ...
            data_c = default_filter_colours.get(
                plot_filters[filt].lower(), def_data_colours[cnt]
            )
            plot_observations(
                ax_sum,
                transient,
    ...
    for cnt, filt in enumerate(filter_names):
        ...
        obs_times, obs_unc = (
            ...
            transient.light_curve_uncertainties,
        )
        ...
        if det_times.size > 0:  ## show scatter
            ...
            dylim = (min(delta_ylim[0], lo - pad), max(delta_ylim[1], hi + pad))
            ax_delta.set_ylim(dylim)

            ax_delta.scatter(
                det_times, normalized_residual, color=fit_color, marker=marker
            )
            ...
            if ax_sum.get_legend():
                ...
                ax_sum.plot(
                    [], [], label=rf"$\chi^2$ / d.o.f. = {round(chi2_dict[filt], 2)}"
                )
                ax_sum.legend(
...
def init_em_analysis_plot(plot_filters, ncols):
    ...
    for cnt, filt in enumerate(filter_names):
        ...
        ax_delta = divider.append_axes("bottom", size="40%", sharex=ax_sum, pad=0.1)
...
def plot_bestfit_with_errors(
    ...
    if sub_model_plot_props is not None:
        ...
        for model_name, prop_dict in sub_model_plot_props.items():
            ...
            ax_sum.plot(plot_times, mag_plot, color=color, linewidth=3, linestyle="--")
            ax_sum.fill_between(
                plot_times,
                mag_plot + mag_err,
                mag_plot - mag_err,
...
def visualise_model_performance(
    ...
    data["redshift"] = 0
    plotName = (
        Path(training_model.outdir)
        / f"injection_{training_model.model}_lightcurves.png"
    )
...
def basic_photo_lc_plot(
    ...
    **kwargs,
    ...
    fig.text(0.01, 0.5, ylabel, fontsize=fontsize, va="center", rotation="vertical")
...
def adjust_observations(ax, transient, filt, xlim, fix_ylim):
    ...
    if fix_ylim:
        ...
        use_ylim = (max(add_ylim[0], old_ylim[0]), min(add_ylim[1], old_ylim[1]))
...
def analysis_plot_geometry(filters_to_plot, ncols=2):
    ...
    figsize = (
        1.5 * (lspace + wpanel * ncols + wspace * (ncols - 1) + trspace),
