...
def em_time_parsing(parser):
    ...
    em_time_parser.add_argument(
        ...
        default=np.inf,
        type=float,
        help="Cutoff time (relative to the trigger time) after which data points will not be considered. If not provided, the entire lightcurve will be evaluated",
    )
    em_time_parser.add_argument(
        "--data-tmin",
        default=0.0,
        type=float,
        help="Cutoff time (relative to the trigger time) before which data points will note be considered. If not provided, the default is 0.",
    )
    return parser
...
def basic_em_only_parsing(parser):
    ...
    parser.add_argument(
        "--plot", action="store_true", help="create characteristic plot"
    )
    parser.add_argument(
        "--verbose", action="store_true", help="print out log likelihoods"
    )
    return parser
...
def basic_em_only_analysis_parsing(parser):
    ...
    parser.add_argument(
        "--light-curve-data",
        "--data",
        help="Path to data in [time filter magnitude error] format, time format will be inferred, but can be explicitly adjusted with --time-format. If not given, will try to generate data from the injection file.",
    )
    parser.add_argument(
        "--time-format",
...
def em_model_parsing(parser):
    ...
    em_model_parser.add(
        "--filters",
        nargs="*",
        help="A list of filters to use (e.g. g,r,i)."
    ...
    em_model_parser.add_argument(
        ...
        "--gptype",
        default="keras",
        help="Interpolation library to be used for EM "
    ...
    em_model_parser.add_argument(
        "--absolute", action="store_true", help="Use Absolute Magnitude?"
    )

    em_model_parser.add_argument(
        "--svd-path",
        default=str(DEFAULT_MODELS_HOME / "svdmodels"),
...
def em_injection_parsing(parser):
    ...
    lc_injection_parser.add(
        "--injection-model-args",
        type=yaml_parse,
    ...
    lc_injection_parser.add_argument(
        "--prompt-collapse",
        action="store_true",  # FIXME: to be deprecated!
...
def modified_em_prior_parsing(parser):
    ...
    mod_em_prior_parser.add_argument(
        ...
        help=(
            ...
            "observer-frame for a Galactic-foreground E(B-V) "
            "(SFD/Planck-bounded priors)."
        ),
    )
    return parser
...
def bolometric_parser(parser):
    ...
    parser.add_argument(
        "--em-model", "--model", help="Name of the transient model to be used"
    )
...
def svd_model_benchmark_parser(parser):
    ...
    parser.add_argument(
        "--ncpus", type=int, default=1, help="Number of CPU to be used (default: 1)"
    )
    parser.add_argument(
        "--outdir", default="benchmark_output", help="Path to the output directory"
    )
    parser.add_argument(
        "--plot", action="store_false", help="Create histogram plots (default: True)"
    )
...
def multi_lc_parser(parser):
    ...
    parser.add_argument(
        "--modeldir", default="model", help="directory where data files are located"
    )
    parser.add_argument(
        "--file-type",
        type=nonestr,
    ...
    parser.add_argument(
        "--lcdir", default="lcs", help="output directory for generated lightcurves"
    )
    parser.add_argument(
        "--doLbol", action="store_true", help="extract bolometric lightcurves"
    )
    parser.add_argument(
        "--doAB", action="store_true", help="extract photometric lightcurves"
    )
    parser.add_argument(
        "--doSmoothing",
        action="store_true",
...
def lc_grid_parser(parser):
    ...
    parser.add_argument(
        "--base-filename", default="lcs", help="Base name of file to save new grid(s)"
    )
    parser.add_argument(
        "--factor",
        type=int,
        default=10,
        help="Factor for downsampling or fragmenting (default: 10)",
    )
    parser.add_argument(
        "--downsample",
        "--do-downsample",
    ...
    parser.add_argument(
        "--shuffle", action="store_true", help="Whether to shuffle input before action"
    )
    parser.add_argument(
        "--remove",
        action="store_true",
        help="Delete all created directories in the end?",
    )
    parser.add_argument(
        "--random-seed", type=int, default=42, help="Random seed for numpy"
    )
    return parser
...
def multi_config_parser(parser):
    ...
    parser.add_argument(
        "--config", help="Name of the configuration file containing parameter values."
    )
    parser.add_argument(
        "--parallel", action="store_true", help="To run multiple configs in parallel"
    )
    parser.add_argument(
        "-p",
        "--process",
