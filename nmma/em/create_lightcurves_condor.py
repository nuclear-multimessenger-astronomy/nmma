...
def main():
    ...
    lc_generation = check_output(["which", "light_curve_generation"]).decode().replace("\n","")

    number_jobs = int(np.ceil(len(injection_dict['injections'])/args.lightcurves_per_job))
    ...
    for ii in range(number_jobs):
        ...
        fid.write('RETRY %d 3\n'%(job_number))
        fid.write('VARS %d jobNumber="%d" INJRANGE="%d,%d"\n'%(job_number, job_number, injection_min, injection_max))
        fid.write('\n\n')
        job_number = job_number + 1

        fid1.write('%s --model %s --svd-path /home/%s/gwemlightcurves/output/svdmodels --outdir %s --label %s --injection-detection-limit 24.1,25.0,25.0,25.3,24.5,23.0,23.2,22.6,22.6 --injection %s --injection-range %d,%d --filters u,g,r,i,z,y,J,H,K --plot\n' % (lc_generation, args.model, os.environ["USER"], args.outdir, args.label, args.injection, injection_min, injection_max))
    ...
    fid.write('arguments = --model %s --svd-path /home/%s/gwemlightcurves/output/svdmodels --outdir %s --label %s --injection-detection-limit 24.1,25.0,25.0,25.3,24.5,23.0,23.2,22.6,22.6 --injection %s --injection-range $(INJRANGE) --filters u,g,r,i,z,y,J,H,K --plot\n' % (args.model, os.environ["USER"], args.outdir, args.label, args.injection)) 
