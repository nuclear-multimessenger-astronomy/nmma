...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Calculate the combination and seperate trend of estimated Hubble constant with GW and EM input"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to the GW posterior samples directory, the samples files are expected to be in the format of posterior_samples_{i}.dat",
    )
    parser.add_argument(
        "--EMsamples",
        metavar="PATH",
        type=str,
        required=True,
        help="Same as the GW samples but for EM samples",
    )
    ...
    parser.add_argument(
        ...
        required=False,
        default=6000,
        help="Number of posterior samples to be drawn during the resampling (default: 6000)",
    )
    parser.add_argument(
        "--N-reordering",
        type=int,
    ...
    assert (
        args.detectable or args.Nevent
    ...
    for i in event_to_loop:
        ...
        z_true = (
            H0_true * distance_injected * (1e3 / scipy.constants.c)
        )  # the factor 100 is to convert speed of light to km s^-1
        ...
        velocity_samples = (
            redshift_samples * scipy.constants.c / 1e3
        )  # velocity in km s^-1
        H0_samples_EM = (
            velocity_samples / distanceEM
    ...
    for _ in tqdm(range(args.N_reordering)):
        ...
        for idx, i in enumerate(index):
            if idx == 0:
                ...
                logprob_combined_GW = 0.0
                logprob_combined_EM = 0.0
                logprob_combined_total = 0.0
                ...
                logprob_combined_total = (
                    logprob_combined_GW
                    + logprob_combined_EM
                    + 3 * np.log(H0_prior_samples)
                )
                logprob_combined_total -= scipy.special.logsumexp(
                    logprob_combined_total
                )
                ...
                logprob_combined_GW = logprob_combined_GW + probs_GW[i].logpdf(
                    H0_prior_samples
                )
                ...
                logprob_combined_EM = logprob_combined_EM + probs_EM[i].logpdf(
                    H0_prior_samples
                )
                ...
                logprob_combined_total = (
                    logprob_combined_total + logprob_combined_GW + logprob_combined_EM
                )
                logprob_combined_total -= scipy.special.logsumexp(
                    logprob_combined_total
                )
        ...
        for i in range(len(prob_cumprod_GW)):
            ...
            GW_kde = scipy.stats.gaussian_kde(
                H0_prior_samples, weights=prob_cumprod_GW[i]
            )
            ...
            EM_kde = scipy.stats.gaussian_kde(
                H0_prior_samples, weights=prob_cumprod_EM[i]
            )
            ...
            total_kde = scipy.stats.gaussian_kde(
                H0_prior_samples, weights=prob_cumprod_total[i]
            )
