...
def get_parser():
    ...
    parser.add_argument(
        "--parameters", type=str, required=True, help="Path to the parameters file"
    )
    parser.add_argument(
        "--tmin",
        type=float,
...
def main(args=None):
    ...
    parameters = pd.read_csv(args.parameters, header=0, delimiter=" ").to_dict(
        "records"
    )
    ...
    if args.plot:
        ...
        lightcurve_colors = ["coral", "cornflowerblue"]
