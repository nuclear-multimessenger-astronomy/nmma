...
def main():
    ...
    parser = argparse.ArgumentParser(description="Calculate the trend of estimated R14 with GW+EM input")
    parser.add_argument(
        "--outdir", 
        metavar="PATH",
        type=str, 
    ...
    for i in range(1, args.Neos + 1):
        m, r = np.loadtxt('{0}/{1}.dat'.format(args.EOSpath, i), usecols=[1, 0], unpack=True)
    ...
    for i in detectable:
        try:
            sample = pd.read_csv('{0}/{1}/posterior_samples.dat'.format(args.GWEMsamples, i), header=0, delimiter=' ')
    ...
    df.to_csv('{0}/GW_EM_R14trend_{1}.dat'.format(args.outdir, args.label), index=False, sep=' ')
    ...
    data_GWEM = pd.read_csv('{0}/GW_EM_R14trend_{1}.dat'.format(args.outdir, args.label), header=0, delimiter=' ')
    data_GW = pd.read_csv('{0}/GW_R14trend.dat'.format(args.gwR14trend), header=0, delimiter=' ')
    ...
    plt.savefig('{0}/R14_trend_GW_EM_{1}.pdf'.format(args.outdir, args.label), bbox_inches='tight')
