...
def main():
    ...
    parser = argparse.ArgumentParser(description="Inference on binary source parameters with kilonova ejecta posterior and GW source posterior given.")

    parser.add_argument("--outdir", 
        type=str, 
        required=True,  
        help="The running directory"
    )
    parser.add_argument(
        "--GWsamples",
        type=str,
    ...
    parser.add_argument("--total-ejecta-mass", 
        action='store_true', 
    ...
    if not args.detections_file is None:
    ...
    gwem_resampling = check_output(["which", "gwem_resampling"]).decode().replace("\n","")
    ...
    for ii in events:
        ...
        gwsamples = os.path.join(args.GWsamples, 'inj_PhD_posterior_samples_%d.dat' % ii)
        if not os.path.isfile(gwsamples):
            print(f'missing {gwsamples}... continuing')
            continue

        emsamples = os.path.join(args.EMsamples, '%d/injection_Bu2019lm_posterior_samples.dat' % ii)
        ...
        fid.write('RETRY %d 3\n'%(job_number))
        fid.write('VARS %d jobNumber="%d" outdir="%s" GWsamples="%s" EMsamples="%s"\n'%(job_number, job_number, outdir, gwsamples, emsamples))
        fid.write('\n\n')
        job_number = job_number + 1

        if args.total_ejecta_mass:
            fid1.write('%s --outdir %s --EMsamples %s --GWsamples %s --EOS %s --nlive 8192 --GWprior %s --EMprior %s --total-ejecta-mass --Neos %d\n' % (gwem_resampling, outdir, emsamples, gwsamples, args.EOSpath, args.GWprior, args.EMprior, args.Neos))
        else:
            fid1.write('%s --outdir %s --EMsamples %s --GWsamples %s --EOS %s --nlive 8192 --GWprior %s --EMprior %s --Neos %d\n' % (gwem_resampling, outdir, emsamples, gwsamples, args.EOSpath, args.GWprior, args.EMprior, args.Neos))
    ...
    if args.total_ejecta_mass:
        fid.write('arguments = --outdir $(outdir) --EMsamples $(EMsamples) --GWsamples $(GWsamples) --EOS %s --nlive 8192 --GWprior %s --EMprior %s --total-ejecta-mass --Neos %d\n' % (args.EOSpath, args.GWprior, args.EMprior, args.Neos))
    else:
        fid.write('arguments = --outdir $(outdir) --EMsamples $(EMsamples) --GWsamples $(GWsamples) --EOS %s --nlive 8192 --GWprior %s --EMprior %s --Neos %d\n' % (args.EOSpath, args.GWprior, args.EMprior, args.Neos))
