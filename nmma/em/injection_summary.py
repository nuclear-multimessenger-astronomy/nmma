...
def loglike_EOS_BNS(cube, ndim, nparams):
    ...
    vals = np.array([complete_parameters['log10_mej_dyn'],
                     complete_parameters['log10_mej_wind']]).T
...
def main():
    ...
    parser.add_argument("--H0", action="store_true", default=False, help="do H0 analysis")
    parser.add_argument("--eos", action="store_true", default=False, help="do EOS analysis")
    parser.add_argument("--ejecta", action="store_true", default=False, help="do ejecta analysis")
    ...
    radius, mass, Lambda = np.loadtxt(args.eos_file,
    ...
    global Neos
    Neos = len(filenames)
    conversion = MultimessengerConversion(args.eos_dir, Neos,
                                          args.binary_type)
    ...
    for EOSIdx in range(0, Neos):
        ...
        EOS_data[EOSIdx]["R14"] = interp.interp1d(EOS_data[EOSIdx]["M"],
                                                  EOS_data[EOSIdx]["R"])(1.4)
    ...
    if not os.path.isfile(pcklFile):
        ...
        for index, row in dataframe_from_inj.iterrows():
            ...
            log10_mej_wind = row["log10_mej_wind"]
            log10_mej_dyn = row["log10_mej_dyn"]               
            #if log10_mej_wind < -2:
            #    print('Small ejecta in %d... continuing' % index)
            #    continue
    
            outdir = os.path.join(args.outdir, str(index))
            posterior_file = os.path.join(outdir, 'injection_' + args.model + '_posterior_samples.dat')
            ...
            lc_data = np.genfromtxt(lc_file, dtype=None, delimiter=',', skip_header = 1, encoding = None)
            ...
            data_out[index]["distance"] = posterior_samples["luminosity_distance"].to_numpy()
            ...
            if args.eos:
                ...
                global kdedir
                pts = np.vstack((posterior_samples["log10_mej_dyn"],
                                 posterior_samples["log10_mej_wind"])).T
                ...
                mMax, rMax, lam1, lam2, r1, r2 = EOS2Parameters(interp_mass_radius,
                                                                interp_mass_lambda,
                                                                row['mass_1_source'],  
                                                                row['mass_2_source'])
                ...
                default_parameters = {'luminosity_distance': row['luminosity_distance'],
                                      'chirp_mass': mchirp,
                                      'ratio_epsilon': 1e-20,
                ...
                if not os.path.isfile(multifile):
                    if args.binary_type == "BNS":
                        pymultinest.run(loglike_EOS_BNS, prior_EOS_BNS, n_params, importance_nested_sampling = False, resume = True, verbose = True, sampling_efficiency = 'parameter', n_live_points = n_live_points, outputfiles_basename='%s/2-'%EOSDir, evidence_tolerance = evidence_tolerance, multimodal = False)
                    elif args.binary_type == "NSBH":
                        pymultinest.run(loglike_EOS_NSBH, prior_EOS_NSBH, n_params, importance_nested_sampling = False, resume = True, verbose = True, sampling_efficiency = 'parameter', n_live_points = n_live_points, outputfiles_basename='%s/2-'%EOSDir, evidence_tolerance = evidence_tolerance, multimodal = False)
                ...
                lambda_tilde = lambda_1_lambda_2_to_lambda_tilde(row["lambda_1"],
                                                                 row["lambda_2"],
                                                                 mass_1_source,
                                                                 mass_2_source)
                R14_approx = mchirp * np.power(lambda_tilde / 0.0042, 1. / 6.) * lal.MRSUN_SI / 1e3
                ...
                for ii, (q, eos, alpha, zeta, logl) in enumerate(data):
                    ...
                    lambda_tilde = lambda_1_lambda_2_to_lambda_tilde(complete_parameters["lambda_1"],
                ...
                q_EM, EOS_EM, alpha, zeta, loglikelihood = data[:,0], data[:,1], data[:,2], data[:,3], data[:,4]
                q_EM = 1/q_EM   
                ...
                lambda_tilde_GW = ss.truncnorm.rvs(a, b, loc=my_mean, scale = my_std,
                                        size=1000)
                data_out[index]["lambda_tilde_GW"] = lambda_tilde_GW         

                R14_GW = mchirp * np.power(lambda_tilde_GW / 0.0042, 1. / 6.) * lal.MRSUN_SI / 1e3
                ...
                lambda_tilde_EM_hist, bin_edges = np.histogram(data_out[index]["lambda_tilde_EM"], bins=bins, density=True)
                bins = (bin_edges[1:]+bin_edges[:-1])/2.0
                bins = np.linspace(0, 5000, 201)
                lambda_tilde_GW_hist, bin_edges = np.histogram(data_out[index]["lambda_tilde_GW"], bins=bins, density=True)
                bins = (bin_edges[1:]+bin_edges[:-1])/2.0

                kdedir_lambda_tilde_EM = greedy_kde_areas_1d(data_out[index]["lambda_tilde_EM"])
                kdedir_lambda_tilde_GW = greedy_kde_areas_1d(data_out[index]["lambda_tilde_GW"])

                data_out[index]["kdedir_lambda_tilde_EM"] = kdedir_lambda_tilde_EM
                data_out[index]["kdedir_lambda_tilde_GW"] = kdedir_lambda_tilde_GW

                max_lambda_tilde = np.max([np.max(lambda_tilde_EM_hist), np.max(lambda_tilde_GW_hist)])
                ...
                if not os.path.isfile(plotName):
                    ...
                    ax.set_xlim([0, 5000])
                    ax.plot(bins, lambda_tilde_EM_hist, color=color1, alpha=1,
                            linestyle='-',linewidth=3, zorder=10)
                    ax.plot(bins, lambda_tilde_GW_hist,color=color2,alpha=1,
                            linestyle='-',linewidth=3, zorder=0)
                    ax.plot([data_out[index]["lambda_tilde"],
                             data_out[index]["lambda_tilde"]],
                            [0, max_lambda_tilde], color='k', linestyle='--')
                    plt.savefig(plotName)
                    plt.close()

                labels = [r"$q$", r"$\alpha$", 
                          r"$\log_{10} \zeta$", r"$R_{\rm 1.4}$"]
                data = np.vstack((q_EM, alpha, zeta, R14_EM)).T
       
                if args.binary_type == "BNS":
                    truths = [data_out[index]["mass_ratio"], row['alpha'],
                              np.log10(row['ratio_zeta']), R14_true]
                elif args.binary_type == "NSBH":
                    truths = [data_out[index]["mass_ratio"], row['log10_alpha'],
                              np.log10(row['ratio_zeta']), R14_true]
                ...
                if not os.path.isfile(multifile):
                    pymultinest.run(loglike_R14, prior_R14, n_params, importance_nested_sampling = False, resume = True, verbose = True, sampling_efficiency = 'parameter', n_live_points = n_live_points, outputfiles_basename='%s/3-'%EOSDir, evidence_tolerance = evidence_tolerance, multimodal = False)
                data = np.loadtxt(multifile)
                EOS_GW = data[:,0]    

                myclip_a, myclip_b = 0, 1
                my_mean, my_std = data_out[index]["mass_ratio"], 0.15*data_out[index]["mass_ratio"]
                a, b = (myclip_a - my_mean) / my_std, (myclip_b - my_mean) / my_std
                q_GW = ss.truncnorm.rvs(a, b, loc=my_mean, scale = my_std,
                                        size=len(EOS_GW))
                data_out[index]["q_GW"] = q_GW

                bins = np.linspace(0.1, 1, 31)
                q_EM_hist, bin_edges = np.histogram(data_out[index]["q_EM"], bins=bins, density=True)
                bins = (bin_edges[1:]+bin_edges[:-1])/2.0
                bins = np.linspace(0.1, 1, 31)
                q_GW_hist, bin_edges = np.histogram(data_out[index]["q_GW"], bins=bins, density=True)
                bins = (bin_edges[1:]+bin_edges[:-1])/2.0
                ...
                if not os.path.isfile(plotName):
                    ...
                    ax.set_xlim([0.1, 1])
                    ax.plot(bins, q_EM_hist, color=color1, alpha=1,
                            linestyle='-',linewidth=3, zorder=10)
                    ax.plot(bins, q_GW_hist,color=color2,alpha=1,
                            linestyle='-',linewidth=3, zorder=0)
                    ax.plot([data_out[index]["mass_ratio"],
                             data_out[index]["mass_ratio"]],
                            [0, maxq], color='k', linestyle='--')
                    plt.savefig(plotName)
                    plt.close()
                ...
                R14_EM_hist, bin_edges = np.histogram(data_out[index]["R14_EM"], bins=bins, density=True)
                bins = (bin_edges[1:]+bin_edges[:-1])/2.0
                bins = np.linspace(7, 17, 201)
                R14_GW_hist, bin_edges = np.histogram(data_out[index]["R14_GW"], bins=bins, density=True)
                bins = (bin_edges[1:]+bin_edges[:-1])/2.0
                ...
                if not os.path.isfile(plotName):
                    ...
                    ax.set_xlim([7, 17])
                    ax.plot(bins, R14_EM_hist, color=color1, alpha=1,
                            linestyle='-',linewidth=3, zorder=10)
                    ax.plot(bins, R14_GW_hist,color=color2,alpha=1,
                            linestyle='-',linewidth=3, zorder=0)
                    ax.plot([data_out[index]["R14"],
                             data_out[index]["R14"]],
                            [0, max_R14], color='k', linestyle='--')
                    plt.savefig(plotName)
                    plt.close()
            ...
            if args.H0:
                ...
                z, zerr = data_out[index]["z"], data_out[index]["zerr"]
                distances = data_out[index]["distance"]
    
                vr_mean, vr_std = z*c, zerr*c
                ...
                data_out[index]["interp_H0_GW"] = interp.interp1d(H0_GW,
                                                                  probdensity_vol,
                ...
                if not os.path.isfile(plotName):
                    ...
                    H0_EMGW_16, H0_EMGW_50, H0_EMGW_84 = compute_constraint(bins, prob_EMGW)
                    print('H0 EM: %.1f +%.1f -%.1f' % (H0_EM_50, H0_EM_84-H0_EM_50, H0_EM_50-H0_EM_16))
                    print('H0 GW: %.1f +%.1f -%.1f' % (H0_GW_50, H0_GW_84-H0_GW_50, H0_GW_50-H0_GW_16))
                    print('H0 EM-GW: %.1f +%.1f -%.1f' % (H0_EMGW_50, H0_EMGW_84-H0_EMGW_50, H0_EMGW_50-H0_EMGW_16))
    
                    filename = "%s/H0.dat"%(H0Dir)
                    fid = open(filename, 'w')
                    fid.write('H0 EM: %.1f +%.1f -%.1f\n' % (H0_EM_50, H0_EM_84-H0_EM_50, H0_EM_50-H0_EM_16))
                    fid.write('H0 GW: %.1f +%.1f -%.1f\n' % (H0_GW_50, H0_GW_84-H0_GW_50, H0_GW_50-H0_GW_16))
                    fid.write('H0 EM-GW: %.1f +%.1f -%.1f' % (H0_EMGW_50, H0_EMGW_84-H0_EMGW_50, H0_EMGW_50-H0_EMGW_16))
                    fid.close()
                    ...
                    maxH0 = np.nanmax([np.max(prob_GW), np.max(prob_EM), np.max(prob_EMGW)])
                    plt.plot([cosmo.H(0).value, cosmo.H(0).value],
                             [0, maxH0], color='k', linestyle='--')
    ...
    if args.ejecta:
        params_pairs = [["log10_mej_wind_true", "log10_mej_wind", r"${\rm log}_{10} (M_{\rm ej,wind})$"],
                        ["log10_mej_dyn_true", "log10_mej_dyn", r"${\rm log}_{10} (M_{\rm ej,dyn})$"],
        ...
        for params_pair in params_pairs:
            ...
            im = ax.pcolormesh(X, Y, hist,
                               shading='auto', cmap='viridis', alpha=0.7)
    ...
    if args.eos:
        ...
        R14_GW170817_16, R14_GW170817_50, R14_GW170817_84 = compute_constraint(bins, prob_GW170817)
        ...
        for nn in range(ntrials):
            ...
            for ii, key in enumerate(keys):
                ...
                R14_EM_16, R14_EM_50, R14_EM_84 = compute_constraint(bins, R14_EM_all_norm)
                R14_GW_16, R14_GW_50, R14_GW_84 = compute_constraint(bins, R14_GW_all_norm)
                R14_EMGW_16, R14_EMGW_50, R14_EMGW_84 = compute_constraint(bins, R14_EMGW_all_norm)
        ...
        l1 = plt.plot([0, ntrials], [R14_GW170817_std, R14_GW170817_std], 'c--', label='GW170817', linewidth=3)
        ...
        plt.plot([R14_true, R14_true],
                 [0, maxR14], color='k', linestyle='--')
    ...
    if args.H0:
        ...
        for nn in range(ntrials):
            ...
            for ii, key in enumerate(keys):
                ...
                H0_EMGW_16, H0_EMGW_50, H0_EMGW_84 = compute_constraint(bins, H0_EMGW_all_norm)
        ...
        l1 = plt.plot([0, ntrials], [planck_std, planck_std], 'g--', label='Planck', linewidth=3)
        l2 = plt.plot([0, ntrials], [shoes_std, shoes_std], 'r--', label='SHOES', linewidth=3)
        l3 = plt.plot([0, ntrials], [superluminal_std, superluminal_std], 'c--', label='GW170817', linewidth=3)
        ...
        plt.ylim([0, 6])
        plt.xlabel('Number of Detections',fontsize=48)
        plt.ylabel(r'1 $\sigma$ Error in $H_0$ [km $\mathrm{s}^{-1}$ $\mathrm{Mpc}^{-1}$]',fontsize=48)
        plt.grid()
        ...
        maxH0 = np.max([np.max(H0_GW_all), np.max(H0_EM_all), np.max(H0_EMGW_all), np.max(prob_GW170817)])
        #plt.plot([cosmo.H(0).value, cosmo.H(0).value],
        ...
        plt.xlabel('$H_0$ [km $\mathrm{s}^{-1}$ $\mathrm{Mpc}^{-1}$]', fontsize=36)
