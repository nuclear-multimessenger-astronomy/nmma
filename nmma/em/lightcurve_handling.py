...
def post_process_bestfit(transient, bestfit_params, args, result=None):
    ...
    chi2_dict, mismatches = compute_chisquare_dict(
        transient, best_mags, observable_times, model_error, verbose=args.verbose
    )

    chi2_dict_raw, _ = compute_chisquare_dict(
        transient,
        best_mags,
        observable_times,
        {filt: 0.0 for filt in model_error.keys()},
    ...
    if getattr(args, "bestfit", False):
        ...
        bestfit_to_write["Magnitudes"] = {
            filt: best_mags[filt].tolist() for filt in transient.observed_filters
        }
        bestfit_to_write["model_error"] = {
            filt: model_error[filt].tolist() for filt in transient.observed_filters
        }
        ...
        bestfit_file = Path(args.outdir) / f"{args.label}_bestfit_params.json"
    ...
    if args.plot:
        ...
        if isinstance(
            lc_model, model.CombinedLightCurveModelContainer
        ) and plot_kwargs.get("plot_submodels", True):
            ...
            for i, sub_model in enumerate(sub_models):
                ...
                for filt in plot_filters.keys():
                    ...
                    plot_errors.append(
                        utils.autocomplete_data(
                            obs_times[i],
                            transient.light_curve_times[filt],
                            model_error_data[filt],
                            extrapolate="constant",
                        )
                    )
        ...
        save_path = plot_kwargs.pop(
            "save_path", Path(args.outdir) / f"{args.label}_bestfit_lightcurves.png"
        )
    ...
    for filt in model_data.keys():
        ...
        if n_finite > 0:
            ...
            if verbose:
                ...
                print(
                    f"for {filt} the length of the detections array is: ",
                    n_finite,
                    "increasing the dof to",
                    dof,
                )
...
def lcs_from_injection_parameters(args=None):
    ...
    light_curve_model = model.create_light_curve_model_from_args(args)
    ...
    if args.plot:
        ...
        plotpath = Path(args.outdir) / f"injection_{identifier}_lc.pdf"
        plot_dict = {
            filt: np.vstack([lc_data[filt]["mag"] for lc_data in mag_ds.values()])
            for filt in filters
        }
...
def make_injection_lightcurve_from_parameters(
    ...
    if injection_outfile.exists():
        try:
            return io.load_em_observations(injection_outfile, format=format)

        except ValueError:
            raise ValueError(
                f"The previous run generated light curves with unreadable content for {injection_outfile}. Consider removing all output files in this format, format then retry."
            )

    data, injection_parameters = make_injection(
        injection_parameters,
        args,
        injection_model=light_curve_model,
        rng=rng,
...
class LightCurveHandler:
    def __init__(self, args):
        ...
        if args.doAB:
            ...
            raise SystemExit(
                "ERROR: Neither --doAB nor --doLbol are enabled. "
                "Please enable at least one."
            )

        self.modeldir = Path(args.modeldir)
        self.lcdir = Path(args.lcdir)
    ...
    def generate_nmma_lcs_from_files(self, directory=None, target_extensions=None):
        ...
        for in_file in tqdm(directory.iterdir()):
            if in_file.suffix not in target_extensions:
                continue
    ...
    def compose_filter_data(self, processed_data):
        ...
        for filt in self.filters:
            bandpass = (
                sncosmo.get_bandpass(filt, 5.0)
                if filt == "ultrasat"
                else sncosmo.get_bandpass(filt)
            )
            m = source.bandmag(bandpass, "ab", self.time)
            data[filt] = {
                "time": self.time,
                "mag": m,
                "mag_error": np.full_like(m, np.nan),
            }
        return data
...
class LANLLightCurveHandler(LightCurveHandler):
    ...
    def process_source(self, i, data):
        ...
        fl = [
            len(data)
            * spec.flux_density_arr.to(u.erg / u.s / u.cm**2 / u.angstrom).value
...
class H5LightCurveHandler(LightCurveHandler):
    ...
    def open_source(self, in_file):
        with h5py.File(in_file) as f:
            ...
            self.wave = np.array(data["wave"]) * (
                1 + self.redshift
            )  # wavelength spec w/ redshift
...
class KasenLightCurveHandler(LightCurveHandler):
    ...
    def compose_filter_data(self, processed_data):
        ...
        if self.smoothing:
            for filt in self.filters:
                prel_mag = utils.autocomplete_data(
                    self.time, self.time, data[filt]["mag"]
                )
                data[filt]["mag"] = savgol_filter(
                    prel_mag, window_length=17, polyorder=3
                )
        return data
...
def resample_lightcurve_grid(args=None):
    ...
    if not gridpath.suffix in [".h5", ".hdf5"]:
        raise ValueError("Resampling currently only supports grid hdf5-files.")

    grid = Grid(args.gridpath, args.base_dirname, args.base_filename, args.random_seed)
...
class Grid:
    def __init__(
        ...
        self.base_dirname = Path(base_dirname)
    ...
    def fragment(self, factor=10, shuffle=False):
        ...
        for i, chunk in enumerate(chunks):
            save_file = save_dir / f"{self.base_filename}_{tag}_{i+1}_of_{factor}.h5"
...
def call_lc_validation(args=None):
    ...
    return validate_lightcurve(
        data_file=args.light_curve_data,
        filters=filters,
...
def validate_lightcurve(
    data_file, filters=None, min_obs=3, cutoff_time=0, verbose=False
):
    """
    ...
    max_time = (
        ...
        if cutoff_time > 0
        else np.max([np.max(filt_dict["time"]) for filt_dict in data.values()])
    )
    ...
    if verbose:
        print(
            f"Lightcurve meets minimum {min_obs} detections in all filters within {max_time-min_time:.2f} days"
        )
