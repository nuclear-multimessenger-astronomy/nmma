...
def main():
    ...
    parser.add_argument("--nlive", metavar='nlive', type=int, required=False, default=1024)
    parser.add_argument("--GWprior", metavar='PATH', type=str, required=True, help='Prior file used for the GW analysis')
    parser.add_argument("--EMprior", metavar='PATH', type=str, required=True, help='Prior file used for the EM eos analysis')
    parser.add_argument("--total-ejecta-mass", action='store_true', help='To run with total ejecta mass, if not activated, the two ejecta are consider seperately')
    ...
    GWsamples = GWsamples.sample(frac=30000 / len(GWsamples), weights=weights, random_state=42)
    ...
    pymulti_kwargs = dict(
        ...
        n_dims=5, n_live_points=args.nlive,
        verbose=True, resume=True,
        seed=42, importance_nested_sampling=False)

    if args.total_ejecta_mass:
        solution = sampler_functions.TotalEjectaMassInference(GWsamples, EMsamples, GWprior, EMprior, args.Neos, args.EOSpath, **pymulti_kwargs)
    else:
        solution = sampler_functions.EjectaMassInference(GWsamples, EMsamples, GWprior, EMprior, args.Neos, args.EOSpath, **pymulti_kwargs)
