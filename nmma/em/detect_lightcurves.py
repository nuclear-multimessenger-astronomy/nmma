...
def main():
    ...
    parser = argparse.ArgumentParser(description="Skymap recovery of kilonovae light curves.")
    parser.add_argument(
        "--injection-file",
        type=str,
        required=True,
    ...
    parser.add_argument("-c", "--configDirectory", help="gwemopt config file directory.", required=True)
    parser.add_argument("--outdir", type=str, required=True, help="Path to the output directory")
    parser.add_argument("--telescope", type=str, default="ZTF", help="telescope to recover")
    parser.add_argument("--tmin", type=float, default=0., help="Days to be started analysing from the trigger time (default: 0)")
    parser.add_argument("--tmax", type=float, default=3., help="Days to be stoped analysing from the trigger time (default: 14)")
    parser.add_argument("--filters", type=str, help="A comma seperated list of filters to use.", default="g,r")
    ...
    parser.add_argument("--parallel", action="store_true", default=False, help="parallel the runs")
    parser.add_argument("--number-of-cores", type=int, default=1, help="Number of cores")
    ...
    for index, row in dataframe_from_inj.iterrows():
        ...
        ra, dec = row['ra'] * 360. / (2 * np.pi), row['dec'] * 360. / (2 * np.pi)
        ...
        exposuretime = ",".join([str(args.exposuretime) for i in args.filters.split(",")])
    ...
    if args.parallel:
        ...
        Parallel(n_jobs=args.number_of_cores)(delayed(os.system)(command) for command in commands)
    else:
    ...
    for index, row in dataframe_from_inj.iterrows():
        ...
        efffile = os.path.join(outdir, f'efficiency.txt')
        if not os.path.isfile(efffile):
            effs.append(0.0)
        else:   
    ...
    (mchirp_det,eta_det,q_det) = ms2mc(dataframe_from_detected["mass_1_source"],
                                       dataframe_from_detected["mass_2_source"])
    (mchirp_miss,eta_miss,q_miss) = ms2mc(dataframe_from_missed["mass_1_source"],
                                          dataframe_from_missed["mass_2_source"])
    ...
    plt.figure()
    plt.scatter(absmag_det, dataframe_from_detected["luminosity_distance"],
                cmap=cmap,
                marker='*',
                c=probs_det, alpha=effs_det, label='Detected')
    plt.scatter(absmag_miss, dataframe_from_missed["luminosity_distance"],
                cmap=cmap,
                marker='o',
                c=probs_miss, alpha=effs_miss, label='Missed')
    ...
    plt.axes(ax1)
    sc1 = plt.scatter(absmag_det, 1-probs_det,
                      ...
                      alpha=effs_det,
                      c=effs_det)
    sc2 = plt.scatter(absmag_miss, 1-probs_miss,
                      ...
                      alpha=effs_miss,
                      c=effs_miss)
    legend_elements = [Line2D([0], [0], marker='o', color='w', label='Missed',
                              markersize=20, markerfacecolor='k'),
                       Line2D([0], [0], marker='*', color='w', label='Found',
                              markersize=20, markerfacecolor='k')]
    if args.binary_type == "BNS":
        plt.xlim([-17.5, -14.0])
    elif args.binary_type == "NSBH":
    ...
    plt.barh(bins[:-1], hist[:-1], height=np.diff(bins), log=True, align='center', facecolor='w', edgecolor='r', label="Missed")
    hist, bin_edges = np.histogram(1-probs_det, bins=yedges, density=False)
    bins = (bin_edges[1:] + bin_edges[:-1])/2.0
    plt.barh(bins[:-1], hist[:-1], height=np.diff(bins), log=True, align='center', facecolor='w', edgecolor='g', label="Detected")
    plt.legend()
    plt.xlabel('Counts')
    #plt.xlim([0.02, 50])
    plt.ylim([0.001, 1.0])
    ...
    bins = (bin_edges[1:] + bin_edges[:-1])/2.0
    plt.bar(bins[:-1], hist[:-1], width=np.diff(bins), align='center', facecolor='w', edgecolor='r', label="Missed")
    hist, bin_edges = np.histogram(absmag_det, bins=xedges, density=False)
    bins = (bin_edges[1:] + bin_edges[:-1])/2.0
    plt.bar(bins[:-1], hist[:-1], width=np.diff(bins), align='center', facecolor='w', edgecolor='g', label="Detected")
    plt.legend()
    plt.ylabel('Counts')
    if args.binary_type == "BNS":
        plt.xlim([-17.5, -14.0])
