...
def main():
    ...
    parser.add_argument(
        "--template-file",
        type=str,
        help="The template file to be used"
    )
    parser.add_argument(
        "--hdf5-file",
        type=str,
        help="The hdf5 file to be used"
    )
    parser.add_argument(
        "--coinc-file",
        type=str,
    ...
    parser.add_argument("--filters", type=str, help="A comma seperated list of filters to use (e.g. g,r,i). If none is provided, will use all the filters available", default="u,g,r,i,z,y,J,H,K")
    ...
    parser.add_argument("--joint-light-curve", help="Flag for using both kilonova and GRB afterglow light curve", action="store_true")
    ...
    parser.add_argument("--injection-detection-limit", metavar="mAB", type=str, default=None, help="The highest mAB to be presented in the injection data set, any mAB higher than this will become a non-detection limit. Should be comma delimited list same size as injection set.") ## need to add default behavior handling

    parser.add_argument("--absolute",action="store_true",default=False,help="Absolute Magnitude")
    ...
    if args.joint_light_curve:
        ...
        if args.model != 'nugent-hyper' or args.model != 'salt2':
            ...
            kilonova_kwargs = dict(model=args.model, svd_path=args.svd_path,
        ...
        if args.model == 'TrPi2018':
            ...
            light_curve_kwargs = dict(model=args.model, sample_times=sample_times,
    ...
    for EOSIdx in range(0, Neos):
        ...
        EOS_data[EOSIdx]["interp_mass_lambda"] = interp.interp1d(EOS_data[EOSIdx]["M"],
                                                                 EOS_data[EOSIdx]["Lambda"])
        EOS_data[EOSIdx]["interp_mass_radius"] = interp.interp1d(EOS_data[EOSIdx]["M"],
                                                                 EOS_data[EOSIdx]["R"])
    ...
    if args.template_file is not None:
        ...
        data_out['chi_eff'] = ((data_out['m1'] * data_out['a1'] +
                                data_out['m2'] * data_out['a2']) /
        ...
        dist_mean, dist_std = distance.parameters_to_marginal_moments(skymap['PROB'],skymap['DISTMU'],skymap['DISTSIGMA'])
    ...
    idys = np.random.choice(np.arange(len(data_out['m1'])), args.Nmarg, p=data_out['weight']/np.sum(data_out['weight']))
    ...
    for ii in range(args.Nmarg):
        ...
        mMax, rMax, lam1, lam2, r1, r2 = EOS2Parameters(EOS_data[idx]["interp_mass_radius"],
                                                        EOS_data[idx]["interp_mass_lambda"],
                                                        m1, m2)
        params = {'luminosity_distance': dist,
                  'chirp_mass': mchirp,
                  'ratio_epsilon': 1e-20,
                  'theta_jn': theta_jn,
                  'a_1': a1,
    ...
    if args.plot:
        ...
        plt.xlabel('log10(Ejecta Mass / $M_\odot$)')
        ...
        for ii, filt in enumerate(filts):
            ...
            tt = np.tile(sample_times, (data_out.shape[0], 1))
            ...
            ax.plot(sample_times, np.nanpercentile(data_out, 50, axis=0), c='k', linestyle='--')
        ...
        fig.text(0.01, 0.5, 'Absolute Magnitude', va='center', rotation='vertical', fontsize=30)
