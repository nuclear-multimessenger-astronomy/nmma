...
def source_frame_masses(converted_parameters, added_keys):
    ...
    if 'redshift' not in converted_parameters.keys():
        ...
        converted_parameters['redshift'] = \
            luminosity_distance_to_redshift(distance)
...
def EOS2Parameters(interp_mass_radius, interp_mass_lambda, mass_1_source, mass_2_source):
    ...
    if mass_1_source < minimum_mass or mass_1_source > TOV_mass:
        lambda_1 = np.array([0.])
        radius_1 = np.array([2. * mass_1_source * lal.MRSUN_SI / 1e3])
    else:
    ...
    if mass_2_source < minimum_mass or mass_2_source > TOV_mass:
        lambda_2 = np.array([0.])
        radius_2 = np.array([2. * mass_2_source * lal.MRSUN_SI / 1e3])
    else:
...
class NSBHEjectaFitting(object):
    ...
    def chieff2risco(self, chi_eff):
        ...
        Z1 = 1. + (1. - chi_eff**2)**(1. / 3) * ((1 + chi_eff)**(1. / 3) + (1 - chi_eff)**(1. / 3))
        Z2 = np.sqrt(3. * chi_eff**2 + Z1**2.)

        return 3. + Z2 - np.sign(chi_eff) * np.sqrt((3. - Z1) * (3. + Z1 + 2. * Z2))

    def remnant_disk_mass_fitting(self, mass_1_source, mass_2_source, compactness_2, chi_eff,
                                  a=0.40642158, b=0.13885773,
                                  c=0.25512517, d=0.761250847):

        mass_ratio_invert = mass_1_source / mass_2_source
        symm_mass_ratio = mass_ratio_invert / np.power(1. + mass_ratio_invert, 2.)

        risco = self.chieff2risco(chi_eff)
        bayon_mass_2 = mass_2_source * (1. + 0.6 * compactness_2) / (1. - 0.5 * compactness_2)

        remant_mass = a * np.power(symm_mass_ratio, -1. / 3.) * (1. - 2. * compactness_2)
        remant_mass += -b * risco / symm_mass_ratio * compactness_2 + c

        remant_mass = np.maximum(remant_mass, 0.)

        remant_mass = np.power(remant_mass, 1. + d)

        remant_mass *= bayon_mass_2

        return remant_mass

    def dynamic_mass_fitting(self, mass_1_source, mass_2_source, compactness_2, chi_eff,
                             a1=7.11595154e-03, a2=1.43636803e-03,
                             a4=-2.76202990e-02,
                             n1=8.63604211e-01, n2=1.68399507):
        ...
        mdyn = a1 * np.power(mass_ratio_invert, n1) * (1. - 2. * compactness_2) / compactness_2
        mdyn += -a2 * np.power(mass_ratio_invert, n2) * risco + a4
        mdyn *= bayon_mass_2

        mdyn = np.maximum(0., mdyn)
    ...
    def ejecta_parameter_conversion(self, converted_parameters, added_keys):
        ...
        mdyn_fit = self.dynamic_mass_fitting(mass_1_source, mass_2_source, compactness_2, chi_eff)
        remnant_disk_fit = self.remnant_disk_mass_fitting(mass_1_source, mass_2_source, compactness_2, chi_eff)
        mdisk_fit = remnant_disk_fit - mdyn_fit

        log_mdyn_fit = np.log(mdyn_fit)
        log_alpha = converted_parameters['log10_alpha'] * np.log(10.)
        log_mej_dyn = np.logaddexp(log_mdyn_fit, log_alpha)
        log10_mej_dyn = log_mej_dyn / np.log(10.)

        converted_parameters['log10_mej_dyn'] = log10_mej_dyn
        converted_parameters['log10_mej_wind'] = np.log10(converted_parameters['ratio_zeta']) + \
            np.log10(mdisk_fit)

        if isinstance(compactness_2, (list, tuple, pd.core.series.Series, np.ndarray)):
            BH_index = np.where(compactness_2 == 0.5)[0]
            negative_mdisk_index = np.where(mdisk_fit <= 0.)[0]
            ...
            if compactness_2 == 0.5 or mdisk_fit < 0.:
...
class BNSEjectaFitting(object):
    ...
    def log10_disk_mass_fitting(self, total_mass, mass_ratio, MTOV, R16, a0=-1.725,
                                delta_a=-2.337, b0=-0.564, delta_b=-0.437,
                                c=0.958, d=0.057, beta=5.879, q_trans=0.886):
        ...
        log10_mdisk = np.maximum(-3., log10_mdisk)

        return log10_mdisk

    def log10_dynamic_mass_fitting_CoDiMaMe(self, mass_1, mass_2,
        ...
        log10_mdyn = a * (1 - 2 * compactness_1) * mass_1 / compactness_1 + \
            b * mass_2 * np.power(mass_1 / mass_2, n) + d / 2

        log10_mdyn += a * (1 - 2 * compactness_2) * mass_2 / compactness_2 + \
            b * mass_1 * np.power(mass_2 / mass_1, n) + d / 2

        return log10_mdyn

    def dynamic_mass_fitting_KrFo(self, mass_1, mass_2,
                                  compactness_1, compactness_2,
                                  a=-9.3335, b=114.17,
                                  c=-337.56, n=1.5465):
        '''
        ...
        mdyn = mass_1 * (a / compactness_1 + b * np.power(mass_2 / mass_1, n) + c * compactness_1)
        mdyn += mass_2 * (a / compactness_2 + b * np.power(mass_1 / mass_2, n) + c * compactness_2)
        mdyn *= 1e-3
    ...
    def ejecta_parameter_conversion(self, converted_parameters, added_keys):
        ...
        mdyn_fit = self.dynamic_mass_fitting_KrFo(mass_1_source, mass_2_source,
                                                  compactness_1, compactness_2)

        log10_mdisk_fit = self.log10_disk_mass_fitting(total_mass, mass_ratio,
        ...
        log10_E0_MSUN = np.log10(converted_parameters['ratio_epsilon']) + \
            np.log10(1. - converted_parameters['ratio_zeta']) + log10_mdisk_fit
        log10_E0_erg = log10_E0_MSUN + np.log10(lal.MSUN_SI * scipy.constants.c * scipy.constants.c * 1e7)
        converted_parameters['log10_E0'] = log10_E0_erg

        if (isinstance(compactness_1, (list, tuple, pd.core.series.Series, np.ndarray)) and
            len(compactness_1) > 1):
...
class MultimessengerConversion(object):
    ...
    def __init__(self, eos_data_path, Neos, binary_type):
        ...
        for EOSID in range(1, self.Neos + 1):
            radius, mass, Lambda = np.loadtxt('{0}/{1}.dat'.format(self.eos_data_path, EOSID),
    ...
    def convert_to_multimessenger_parameters(self, parameters):
        ...
        converted_parameters, added_keys = source_frame_masses(converted_parameters, added_keys)
        mass_1_source = converted_parameters['mass_1_source']
        mass_2_source = converted_parameters['mass_2_source']

        if 'EOS' in converted_parameters:
            if isinstance(converted_parameters['EOS'], (list, tuple, pd.core.series.Series, np.ndarray)):
                TOV_radius_list = []
                TOV_mass_list = []
                lambda_1_list = []
                lambda_2_list = []
                ...
                for i in range(0, len(EOSID)):
                    ...
                    interp_mass_lambda, interp_mass_radius = self.eos_interp_dict[EOSID[i]]

                    TOV_mass, TOV_radius, lambda_1, lambda_2, radius_1, radius_2 = \
                        EOS2Parameters(interp_mass_radius, interp_mass_lambda,
            ...
            added_keys = added_keys + ['lambda_1', 'lambda_2', 'TOV_mass', 'TOV_radius',
                                       'radius_1', 'radius_2', 'R_14', 'R_16']

            converted_parameters, added_keys = self.ejecta_parameter_conversion(converted_parameters,
                                                                                added_keys)

            theta_jn = converted_parameters['theta_jn']
            converted_parameters['KNtheta'] = 180 / np.pi * np.minimum(theta_jn, np.pi - theta_jn)
            converted_parameters['inclination_EM'] = converted_parameters['KNtheta'] * np.pi / 180.
...
class MultimessengerConversionWithLambdas(object):
    ...
    def convert_to_multimessenger_parameters(self, parameters):
        ...
        compactness_1 = 0.371 - 0.0391 * log_lambda_1 + 0.001056 * log_lambda_1 * log_lambda_1
        compactness_2 = 0.371 - 0.0391 * log_lambda_2 + 0.001056 * log_lambda_2 * log_lambda_2

        converted_parameters['radius_1'] = mass_1_source / compactness_1 * lal.MRSUN_SI / 1e3
        converted_parameters['radius_2'] = mass_2_source / compactness_2 * lal.MRSUN_SI / 1e3

        chirp_mass_source = component_masses_to_chirp_mass(mass_1_source,
                                                           mass_2_source)
        lambda_tilde = lambda_1_lambda_2_to_lambda_tilde(lambda_1, lambda_2,
                                                         mass_1_source, mass_2_source)

        converted_parameters['R_16'] = chirp_mass_source * np.power(lambda_tilde / 0.0042, 1. / 6.) * lal.MRSUN_SI / 1e3
