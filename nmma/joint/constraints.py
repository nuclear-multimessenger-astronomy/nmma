...
class NICERConstraint(object):
    def __init__(self, NICER_path=None):
        ...
        if NICER_path is None:
            self.NICER_path = os.path.join(
                os.path.dirname(__file__), "NICER/J0030_3spot_RM.txt"
            )
        else:
    ...
    def __repr__(self):
        return (
            self.__class__.__name__
            + " with the observation of PSR J0030+0451 "
            + "with three potentially overlapping ovals. Data taken from {}".format(
                self.NICER_path
            )
        )
    ...
    def log_likelihood(self, parameters):
        ...
        masses = np.linspace(1.0, 2.0, num=1000)
