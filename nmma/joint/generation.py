...
def write_complete_config_file(parser, args, inputs, remove_none=True):
    ...
    args.submit = False
    if remove_none:
        non_default_args = {k: v for k, v in args.__dict__.items() if v is not None}
    ...
    def adjust_priors_and_data(self, args, logger):
        ...
        if self.injection_parameters:
            try:
                ...
                logger.warning(
                    "eos and ejecta fitting failed for injection parameters. Continuing without conversion."
                )
                ...
                args.injection_dict = {
                    k: float(v) for k, v in converted_injection.items()
                }
                if "gw" in messengers:
                    self.gw_inputs = NMMAGravitationalWaveInput(args, self.unknown_args)
                    data_dump |= dict(
        ...
        if args.em_model or args.em_transient_class:
            ...
            if self.injection_parameters:
                ...
                for param in injection_model.model_parameters:
                    if param not in self.injection_parameters:
                        try:
                            self.injection_parameters[param] = converted_injection[
                                param
                            ]
                        except KeyError:
                            raise KeyError(
                                f"Required parameter {param} could not be derived from conversion."
                            )
            ...
            sys_handler = FilterSystematicsHandler(
                filters, args.systematics_file, error_budget=args.em_error_budget
            )
        ...
        logger.info(
            f"Set up data dump with messengers: {self.messengers} "
