...
def pad_the_data(
    ...
    """
    actual_df.iloc[:, actual_df.columns.get_loc("t")] = actual_df.iloc[
        :, actual_df.columns.get_loc("t")
    ].apply(lambda x: x - t_min)
    cat_df = actual_df
    cat_count = len(cat_df)
    prepended_count = 0
    if actual_df["t"].min() >= filler_time_step:
        filler_max_time = (
            actual_df["t"].min() - filler_time_step
        )  # stop one time step before current min
        prepend_filler_df = gen_prepend_filler(
            column_list, filler_data, 0, filler_max_time, filler_time_step
        )
    ...
    if append_count > 0:
        ...
        filler_min_time = (
            int(max_t * steps_per_count) / steps_per_count + filler_time_step
        )  # start at next time step
        append_filler_df = gen_append_filler(
            column_list, filler_data, filler_min_time, append_count
        )
...
def load_in_data(
    data_dir, name, csv_no, num_points=num_points, num_repeats=num_repeats
):
    """
...
def match_fix_to_var(
    data_dir, name1, name2, start, stop, num_points=num_points, num_repeats=num_repeats
):
    """
    ...
    for i in range(start, stop):
        ...
        matched = df_var.merge(
            df_fix,
            left_on=["sim_id", df_var.groupby("sim_id").cumcount()],
    ...
    fixed_data_df = fixed_data_df.iloc[
        0 : (num_batches * num_points * num_repeats), :
    ].copy()
    varied_data_df = varied_data_df.iloc[
        0 : (num_batches * num_points * num_repeats), :
    ].copy()
    for i in range(0, num_batches):
        for j in range(0, num_points * num_repeats):
...
def repeated_df_to_tensor(df_varied, df_fixed, batches):
    ...
    for idx in tqdm(range(0, batches)):
        data_shifted = torch.tensor(
            df_varied.loc[df_varied["batch_id"] == idx]
            .iloc[:, 1:4]
            .values.reshape(num_repeats, num_points, num_channels),
            dtype=torch.float32,
        ).transpose(1, 2)
        data_unshifted = torch.tensor(
            df_fixed.loc[df_fixed["batch_id"] == idx]
            .iloc[:, 1:4]
            .values.reshape(num_repeats, num_points, num_channels),
            dtype=torch.float32,
        ).transpose(1, 2)
