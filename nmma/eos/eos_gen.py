...
def eos_from_nep(
    ...
    def EA_SNM(n):
        xexp = (n - nsat) / (3.0 * nsat)
        return (
            Esat + Ksat * xexp**2 / 2.0 + Qsat * xexp**3 / 6.0 + Zsat * xexp**4 / 24.0
        )
    ...
    # Symmetry energy
    def EA_beta(n, x):
        return EA_SNM(n) + EA_sym(n) * (1 - 2.0 * x)
...
class EOS_with_CSE:
    ...
    def __init__(
        ...
        if not low_density_eos_stiff:
            ...
            assert len(low_density_eos) == len(low_density_eos_stiff), (
                "This requires interpolation. Will be added in the future."
            )
            # fix the seed
    ...
    def __extend(self):
        ...
        for node_index in range(1, self.N_seg + 1):
            ...
            n_val_upper_bound = min(
                n_val_lower_bound + 1.5 * self.n_extend_range / self.N_seg, self.n_lim
            )
            n_val = np.random.uniform(n_val_lower_bound, n_val_upper_bound)
            cs2_val = np.random.uniform(0.0, self.cs2_limit)
            cs2_draw[node_index] = [n_val, cs2_val]

        # add the speed for sound value at n_lim
        cs2_at_n_lim = np.random.uniform(0.0, self.cs2_limit)
        cs2_draw[-1] = [self.n_lim, cs2_at_n_lim]

        # interpolation for cs2(n)
        cs2_extent = interp1d(
            cs2_draw[:, 0], cs2_draw[:, 1], kind="linear", fill_value="extrapolate"
        )
        ...
        sol = solve_ivp(
            dydt,
            (np.log(self.n_connect), np.log(self.n_lim)),
            y0,
            method="DOP853",
            t_eval=np.log(n_high),
            args=(cs2_extent,),
            rtol=1e-7,
            atol=0.0,
        )
    ...
    def __extend_v1(self):
        ...
        n_ext_grid = np.linspace(
            self.n_connect + 1e-4 * self.n_connect, self.n_lim, num=self.N_seg + 1
        )
        ...
        num = 50
        n_high = [
            np.linspace(n_ext_grid[i], n_ext_grid[i + 1], endpoint=False, num=num)
        ...
        for i in range(n_ext_grid.size - 1):
            slope = (c2_ext_grid[i + 1] - c2_ext_grid[i]) / (
                n_ext_grid[i + 1] - n_ext_grid[i]
            )
            c2_high[i, :] = slope * (n_high[i, :] - n_ext_grid[i]) + c2_ext_grid[i]
            mu_ext_grid[i + 1] = mu_ext_grid[i] * np.exp(
                slope
                * (
                    n_ext_grid[i + 1]
                    - n_ext_grid[i]
                    - n_ext_grid[i] * np.log(n_ext_grid[i + 1] / n_ext_grid[i])
                )
                + c2_ext_grid[i] * np.log(n_ext_grid[i + 1] / n_ext_grid[i])
            )
            mu_high[i, :] = mu_ext_grid[i] * np.exp(
                slope
                * (
                    n_high[i, :]
                    - n_ext_grid[i]
                    - n_ext_grid[i] * np.log(n_high[i, :] / n_ext_grid[i])
                )
                + c2_ext_grid[i] * np.log(n_high[i, :] / n_ext_grid[i])
            )
        ...
        self.e_high = (
            cumulative_trapezoid(self.mu_high, self.n_high, initial=0)
            + self.e_at_n_connect
        )
    ...
    def __construct_all_interpolation(self):
        ...
        self.log_energy_density_from_log_pseudo_enthalpy = interp1d(
            np.log(self.h_array),
            np.log(self.e_array),
            kind="linear",
            fill_value="extrapolate",
        ...
        self.log_energy_density_from_log_number_density = interp1d(
            np.log(self.n_array),
            np.log(self.e_array),
            kind="linear",
            fill_value="extrapolate",
        ...
        self.log_pressure_from_log_energy_density = interp1d(
            np.log(self.e_array),
            np.log(self.p_array),
            kind="linear",
            fill_value="extrapolate",
        ...
        self.log_pressure_from_log_number_density = interp1d(
            np.log(self.n_array),
            np.log(self.p_array),
            kind="linear",
            fill_value="extrapolate",
        ...
        self.log_pressure_from_log_pseudo_enthalpy = interp1d(
            np.log(self.h_array),
            np.log(self.p_array),
            kind="linear",
            fill_value="extrapolate",
        ...
        self.log_number_density_from_log_pressure = interp1d(
            np.log(self.p_array),
            np.log(self.n_array),
            kind="linear",
            fill_value="extrapolate",
        ...
        self.log_number_density_from_log_pseudo_enthalpy = interp1d(
            np.log(self.h_array),
            np.log(self.n_array),
            kind="linear",
            fill_value="extrapolate",
        ...
        self.log_number_density_from_log_energy_density = interp1d(
            np.log(self.e_array),
            np.log(self.n_array),
            kind="linear",
            fill_value="extrapolate",
        ...
        self.log_pseudo_enthalpy_from_log_pressure = interp1d(
            np.log(self.p_array),
            np.log(self.h_array),
            kind="linear",
            fill_value="extrapolate",
        ...
        self.log_pseudo_enthalpy_from_log_energy_density = interp1d(
            np.log(self.e_array),
            np.log(self.h_array),
            kind="linear",
            fill_value="extrapolate",
        ...
        self.log_pseudo_enthalpy_from_log_number_density = interp1d(
            np.log(self.n_array),
            np.log(self.h_array),
            kind="linear",
            fill_value="extrapolate",
        ...
        self.log_dedp_from_log_pressure = interp1d(
            np.log(self.p_array),
    ...
    def construct_family(self, ndat=100):
        ...
        if i != ndat - 1:
            ...
            res = minimize_scalar(
                f, method="bounded", bounds=(logpcs[0] * 1.001, logpcs[-1] * 0.999)
            )
            logpmax = res.x
            pmax = np.exp(logpmax)
        ...
        lambdas = 2.0 / 3.0 * ks * np.power(cs, -5.0)
