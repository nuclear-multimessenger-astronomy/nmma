...
SI_to_geometric = scipy.constants.G / np.power(scipy.constants.c, 4.)
...
def TOVSolver(eos, pc_pp):
    ...
    h0 = hc + dh
    h1 = -dh
    r0 = np.sqrt(3. * (-dh) / 2. / np.pi / (ec + 3. * pc))
    r0 *= 1. - 0.25 * (ec - 3. * pc - 0.6 * dedh_c) * (-dh) / (ec + 3. * pc)
    m0 = 4. * np.pi * ec * np.power(r0, 3.) / 3.
    ...
    sol = solve_ivp(tov_ode, (h0, h1), y0, args=(eos,), rtol=1e-3, atol=0.)
