...
class PulsarConstraint(object):
    def __init__(self, pulsar_masses, pulsar_masses_error, EOSpath, Neos):
        ...
        self.pulsar_masses_error = (
            pulsar_masses_error  # list of errors of pulsar masses
        )
    ...
    def __repr__(self):
        return (
            ...
            + " with {} pulsars of masses {} and errors of {}".format(
                len(self.pulsar_masses), self.pulsar_masses, self.pulsar_masses_error
            )
        )
...
class MTOVUpperConstraint(object):
    ...
    def __repr__(self):
        return self.__class__.__name__ + " maxTOV of {} and error of {}".format(
            self.maxMTOV, self.maxMTOV_error
        )
...
class NICERConstraint(object):
    ...
    def __repr__(self):
        return (
            self.__class__.__name__
            + " with the observational NICER data taken from {}".format(self.NICER_path)
        )

    def NICER_logweight(self, NICER_path, EOSpath, Neos):

        # NICER I
        NICER_radius, NICER_mass = np.loadtxt(
            "{0}/J0030_3spot_RM.txt".format(NICER_path),
            skiprows=1,
            usecols=[0, 1],
            unpack=True,
        )
        ...
        # NICER II
        NICER_radius, NICER_mass = np.loadtxt(
            "{0}/NICER_x_XMM_J0740_XPSI_STU_NSX_FIH_radius_mass.txt".format(NICER_path),
            skiprows=1,
            usecols=[0, 1],
            unpack=True,
        )
        ...
        NICER_radius, NICER_mass = np.loadtxt(
            "{0}/NICER+XMM_J0740_RM.txt".format(NICER_path),
            skiprows=1,
            usecols=[0, 1],
            unpack=True,
        )
        ...
        for EOSIdx in tqdm(range(1, Neos + 1)):
            ...
            Mrange = np.linspace(1.0, 2.0, num=100)
            Mrange = Mrange[Mrange < mMax]
            NICER_logpdf = scipy.special.logsumexp(
                NICER_KDE.logpdf((R_M_interp(Mrange), Mrange))
            )

            # NICER II
            Mrange = np.linspace(1.5, 2.5, num=100)
            Mrange = Mrange[Mrange < mMax]
            NICER_XPSI_logpdf = scipy.special.logsumexp(
                NICER_KDE_XPSI.logpdf((R_M_interp(Mrange), Mrange))
            )
            NICER_Maryland_logpdf = scipy.special.logsumexp(
                NICER_KDE_Maryland.logpdf((R_M_interp(Mrange), Mrange))
            )

            NICERII_logpdf = scipy.special.logsumexp(
                [NICER_XPSI_logpdf, NICER_Maryland_logpdf]
            )  # adding the likelihood from XPSI and Maryland (therefore, an average)
...
class JointConstraint(object):
    ...
    def __repr__(self):
        return (
            self.__class__.__name__
            + " for pulsar, maximum TOV mass and NICER weights computation. NICER data taken from {}".format(
                self.NICER_path
            )
        )
    ...
    def total_logweight(
        self, EOSpath, Neos, pulsar_weight, maxMTOV_weight, NICER_weight
    ):
...
def EOSConstraintsLoglikelihood(eos_path, Neos, Constraint):
    ...
    for eos in range(1, Neos + 1):
        radius, mass, Lambda = np.loadtxt(
            "{0}/{1}.dat".format(eos_path, eos), unpack=True, usecols=[0, 1, 2]
        )
...
def EOSSorting(eos_path, out_path, Neos, weights):
    ...
    for sortedIdx in range(0, Neos):
        ...
        os.system(
            "cp {0}/{1}.dat {2}/{3}.dat".format(
                eos_path, originalIdx, out_path, sortedIdx + 1
            )
        )
