...
def marginalised_lightcurve_expectation_from_gw_samples(args=None):
    ...
    conversion = conv.MultimessengerConversion.basic_cbc(
        EoSConverter(args, method="tabulated"), light_curve_model.parameter_conversion
    )

    ## read eos and gw data
    EOS_data, weights, Neos = load_tabulated_macro_eos_set_to_dict(
        args.eos_data, args.eos_weights
    )
    args.Neos = Neos
    ...
    if args.template_file is not None:
        ...
        data_out["dist"] = (
            dist_mean + rng.standard_normal(len(data_out["m1"])) * dist_std
        )
    ...
    for ii in range(args.Nmarg):
        ...
        lightcurve_outfile = outdir / "lc.dat"
        matter_outfile = outdir / "matter.dat"
        if lightcurve_outfile.is_file() and matter_outfile.is_file():
        ...
        params.update({"alpha": alpha, "ratio_zeta": zeta})
        ...
        complete_parameters["trigger_time"] = read_trigger_time(
            complete_parameters, args
        )
    ...
    if args.plot:
        ...
        plotName = Path(args.outdir) / "matter.pdf"
        ...
        plotpath = Path(args.outdir) / "lc.pdf"
        plot_dict = {
            filt: np.vstack([lc_data[filt]["mag"] for lc_data in mag_ds])
            for filt in filters
        }
        ...
        lc_plot_with_histogram(
            filters, plot_dict, times, plotpath, ylim=ylim, fontsize=30
        )
...
def get_all_gw_quantities(data_out):
    try:
        ...
        data_out["m1"], data_out["m2"] = conv.chirp_mass_and_eta_to_component_masses(
