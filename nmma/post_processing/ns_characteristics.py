...
def estimate_observable_trend(prior_dist, posterior_probs, prior_prob, args):
    ...
    for _ in tqdm(range(args.N_reordering)):
        ...
        observable_spread = find_spread_from_resampling(
            R14_resampling,
            prob_cumprod,
            prior_dist,
        ...
        for obs_sub_list, spread_estimate in zip(
            (obs_med, obs_uplim, obs_lowlim), observable_spread
        ):
            obs_sub_list.append(spread_estimate)

    return [
        np.median(sub_array, axis=0) for sub_array in [obs_med, obs_uplim, obs_lowlim]
    ]
...
def main():
    ...
    R14_med, R14_uplim, R14_lowlim = estimate_observable_trend(
        R14_prior, probs, prior_prob, args
    )
    ...
    df.to_csv(f"{args.outdir}/GW_EM_R14trend_{args.label}.dat", index=False, sep=r"\s+")
